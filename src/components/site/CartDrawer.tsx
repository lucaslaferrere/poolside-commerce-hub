    ...
    <Sheet open={isOpen} onOpenChange={(o) => !o && close()}>
      <SheetContent side="right" className="w-full sm:max-w-md flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto -mx-6 px-6 py-4">
          {items.length === 0 ? (
            <div className="h-full grid place-items-center text-center text-neutral-500 py-16">
              <div>
            ...
            <ul className="space-y-3">
              {items.map((item) => (
                <li
                  key={item.id}
                  className="flex gap-3 p-3 rounded-lg border border-neutral-200 bg-white"
                >
                  {item.image_url && (
                    <img
                      src={item.image_url}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium leading-tight line-clamp-2 text-neutral-900">
                      {item.name}
                    </p>
                    ...
                    <div className="mt-2 flex items-center gap-1">
                      ...
                      <Button
                        size="icon"
                        variant="ghost"
                        className="h-7 w-7 ml-auto text-neutral-400 hover:text-danger hover:bg-danger/10"
        ...
        {items.length > 0 && (
          <div className="border-t border-neutral-200 pt-4 space-y-3">
            ...
            <p className="text-xs text-neutral-500">El envío se calcula al finalizar.</p>
            <Button
              className="w-full bg-brand text-brand-foreground hover:bg-brand-hover active:bg-brand-active shadow-none"
              size="lg"
