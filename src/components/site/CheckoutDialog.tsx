...
import { CheckCircle2, CreditCard, Wallet, Truck, ExternalLink, Package } from 'lucide-react';
...
type DeliveryMethod = 'envio' | 'retirar';
...
export function CheckoutDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const sub      = subtotal();
  const shipping = delivery === 'retirar' ? 0 : calcShipping(form.shipping_zip ?? '', sub);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // Shipping fields required only when delivery = envio
    if (delivery === 'envio') {
      const extra: Partial<Record<keyof FormData, string>> = {};
      if (!form.shipping_address?.trim()) extra.shipping_address = 'Requerido';
      if (!form.shipping_city?.trim())    extra.shipping_city    = 'Requerido';
      if (!form.shipping_zip?.trim())     extra.shipping_zip     = 'Requerido';
        ...
        {done ? (
          <div className="text-center py-8">
            ...
            <DialogDescription className="mt-3">
              {delivery === 'retirar'
                ? 'Te contactamos a la brevedad para coordinar el retiro.'
          ...
          <>
            ...
            <form onSubmit={submit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={delivery} onValueChange={(v) => setDelivery(v as DeliveryMethod)} className="grid sm:grid-cols-2 gap-2">
                ...
                {delivery === 'retirar' && (
                  <p className="text-xs text-muted-foreground">Te contactamos para coordinar el retiro.</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={payment} onValueChange={(v) => setPayment(v as PaymentMethod)} className="grid sm:grid-cols-2 gap-2">
                ...
                {payment === 'transferencia' && (
                  <p className="text-xs text-primary">3.5% de descuento extra al confirmar.</p>
                )}
              </div>
              ...
              <div className="bg-muted rounded-lg p-4 space-y-1.5 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  {delivery === 'retirar' ? (
                    <span className="text-primary font-medium">Sin costo</span>
                  ) : shipping > 0 ? (
                ...
                <div className="border-t pt-2 mt-2 flex justify-between font-display font-bold text-lg">
                  ...
                  <span className="text-primary">
                    {formatPrice(total)}
                    {delivery === 'envio' && shipping === 0 && (
                      <span className="text-sm font-normal text-muted-foreground ml-1">+ envío</span>
                    )}
                  </span>
                </div>
              </div>
